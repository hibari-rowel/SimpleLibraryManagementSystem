<script setup lang="ts">
    ...
    type EditUserFormInterface = Omit<UserFormInterface, 'password' | 'confirm_password'>;

    const formData = reactive <EditUserFormInterface> ({
        first_name: '',
        middle_name: '',
        last_name: '',
        email: '',
        ...
        (newUser) => {
            if (newUser) {
                Object.assign(formData, {
                    ...
                    postal_code_address: newUser.postal_code_address || '',
                    state_address: newUser.state_address || '',
                    country_address: newUser.country_address || '',
    ...
    const submitForm = (formData: EditUserFormInterface) => {
        formData.user_id = recordID;
        userStore.updateUser(formData, recordID);
    }
...
<template>
    <base-template :header="header">
        <template v-slot:button>
            <RouterLink :to="'/users/show/' + recordID" class="btn btn-danger font-weight-bolder mr-2"> Cancel </RouterLink>
