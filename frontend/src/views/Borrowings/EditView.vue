<script setup lang="ts">
    ...
    import {CategoryFormInterface} from "../../core/interface/CategoryFormInterface";
    ...
    watch(
        () => borrowingStore.borrowing,
        (newBorrowing) => {
            if (newBorrowing) {
                Object.assign(formData, {
                    name: newBorrowing.name || '',
                    user_id: newBorrowing.user_id || '',
                    ...
                    description: newBorrowing.description || '',
                });
            }
        }
    );
    ...
    const submitForm = (formData: CategoryFormInterface) => {
        borrowingStore.updateBorrowing(formData, recordID);
    }

    onMounted(() => {
        bookStore.getCategoryDropdownList();
        userStore.getCategoryDropdownList();
        borrowingStore.getBorrowing({}, recordID);
    });
</script>
